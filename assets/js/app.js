...
for (var i = 0; i < workHours.length; i++) {
	...
	var $saveButton = $('<button>', { class: 'saveBtn' }).html(
	...
	if (moment(workHours[i], 'hh A').isAfter(currentTime)) {
		$timeblock.addClass('future');
	} else if (moment(workHours[i], 'hh A').isSame(currentTime)) {
